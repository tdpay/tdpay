...
//	$("#pageUnit").val("${pageUnit}").prop("selected", true);
    ...
    <form class="table_layout02_wrap" name="frm2" id="frm2" action="" method="post" enctype="multipart/form-data">
        ...
        <input type="hidden" name="mCommission" id="mCommission" value="${sessionScope.sessionLoginAction.commission-result.commission}"/>				
        ...
        <table class="table_layout02">
            ...
            <thead>
                <tr>
                    ...
                    <!-- <th><button type="button">계좌이체 수수료</button></th>
                    <th><button type="button">가상계좌 수수료</button></th> -->
                    ...
                    <tr>
                        ...
                        <td class="management">
                            <div class="setting_btn_wrap">
                                <button onclick="frmInfo('<c:out value="${result.store_id}" />','<c:out value="${result.role_id}" />','<c:out value="${result.no}" />')"
                                    class="img_set" type="button"></button>
                                <button onclick="frmDel('<c:out value="${result.store_id}" />')" class="img_del" type="button"></button>
                            </div>
                        </td>
                        <td class="id point_link" onclick="frmView('<c:out value="${result.store_id}" />','<c:out value="${result.role_id}" />','<c:out value="${result.no}" />')"><c:out value="${result.store_id}" /></td>
